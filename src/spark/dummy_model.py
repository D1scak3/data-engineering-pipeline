...
class DummyModel:
    ...
    """
    This class is a dummy model, used only to simbolize a model.
    It is a simple random value that is predicted.
    There is no real model fitting or prediction.
    IT'S A DUMMY.
    """
    ...
    def fit(self, data_x, data_y, epochs, batch_size, validation_data, verbose, shuffle):
        self.is_fitted = True
        self.model.fit(x=data_x, 
                       y=data_y,
                       epochs=epochs,
...
if __name__ == "__main__":
    ...
    features = 1
    percent_train = 0.7
    dummy_model = DummyModel(n_steps=steps, n_features=features)

    # read data
    data = pd.read_pickle("../data/long_product_group_id_23")
    # data = pd.read_table("../data/exported/exported_new_data.tsv", on_bad_lines="skip")
    # print(data)
    # input(...)
    ...
    # predict values
    results = dummy_model.predict(test[0])
    # print(results)
